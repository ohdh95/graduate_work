				       ...
				       const struct read_write_emulator_ops *ops)
{
	...
	// struct kvm_memory_slot *slot = kvm_vcpu_gfn_to_memslot(vcpu, addr >> PAGE_SHIFT);
			...
			const struct read_write_emulator_ops *ops)
{
	...
	gpa_t gpa;
	int rc;
	// printk("[%s] gpa:0x%llx(%d) addr:0x%lx\n",__func__, ctxt->gpa_val, ctxt->gpa_available, addr);
	...
	rc = ops->read_write_exit_mmio(vcpu, gpa, val, bytes);
	// printk("\t[%s] gpa:0x%llx(%d) addr:0x%lx\n","after mmio exit", ctxt->gpa_val, ctxt->gpa_available, addr);
	return rc;
			    ...
			    int emulation_type, void *insn, int insn_len)
{
	...
	if (!(emulation_type & EMULTYPE_NO_DECODE)) {
		...
		if (r == EMULATION_OK_RESTART) {
			// printk("EMULATION_OK_RESTART gpa:0x%llx, insn:0x%llx, len:%d r:%d\n", cr2_or_gpa, (insn)?*(u64*)insn:0, insn_len, r);
			// r = 4;
			// printk("have_exception: %d, ctxt->exception.vector: 0x%x\n", ctxt->have_exception, ctxt->exception.vector);
			// ctxt->have_exception = false;
			// ctxt->exception.vector = 0;
			// goto restart;
			...
			// vcpu->mmio_nr_fragments = 0;
			// vcpu->mmio_fragments[0].gpa = cr2_or_gpa;
			// vcpu->run->exit_reason = KVM_EXIT_MMIO;
			// vcpu->run->mmio.phys_addr = cr2_or_gpa;
			return 0;
		}
