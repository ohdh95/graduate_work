			    ...
			    const struct kvm_userspace_memory_region *mem)
{
	...
	if ((mem->memory_size & (PAGE_SIZE - 1)) ||
		...
		printk("mem_sz: %llx", (u64)mem->memory_size);
		return -EINVAL;
	}
		
	if (mem->guest_phys_addr & (PAGE_SIZE - 1)) {
		printk("gpa: %llx", (u64)mem->guest_phys_addr);	
		return -EINVAL;
	}
	...
	if ((mem->userspace_addr & (PAGE_SIZE - 1)) ||
		...
		printk("ua: %llx", (u64)mem->userspace_addr);				
		return -EINVAL;
	}
	...
	if ((mem->memory_size >> PAGE_SHIFT) > KVM_MEM_MAX_NR_PAGES) {
		printk("nr_pgs %llx > %llx", (u64)(mem->memory_size >> PAGE_SHIFT), (u64)KVM_MEM_MAX_NR_PAGES);
		return -EINVAL;
	}
					  ...
					  struct kvm_userspace_memory_region *mem)
{
	if ((u16)mem->slot >= KVM_USER_MEM_SLOTS) {
		printk("mem_slots: %llx >= %llx", (u64)mem->slot, (u64)KVM_USER_MEM_SLOTS);
		return -EINVAL;
	}
			   ...
			   unsigned int ioctl, unsigned long arg)
{
	...
	if (ioctl == KVM_SET_SPTE_FLAG) {
		struct kvm_set_spte_flag kvm_set_spte_flag;
		r = -EINVAL;
		if (copy_from_user(&kvm_set_spte_flag, argp, sizeof(struct kvm_set_spte_flag))){
			printk("KVM_SET_SPTE_FLAG arg err");		
		}
		else {
			r = kvm_arch_vcpu_ioctl_set_spte_flag(vcpu, &kvm_set_spte_flag);
		} 
		return r;
	}
	// else if (ioctl == KVM_GET_ROOT_SPTEP) {
	// 	struct kvm_get_root_sptep root_sptep;
	// 	r = kvm_arch_vcpu_ioctl_get_root_sptep(vcpu, &root_sptep);
	...
	// 	r = kvm_arch_vcpu_ioctl_set_spte_flag(vcpu, &kvm_set_spte_flag);
	// 	// printk("KVM_SET_SPTE_FLAG: ret %d", r);
	// 	break;
	// }
	case KVM_RUN: {
		struct pid *oldpid;
