...
static int kvm_set_user_memory_region(KVMMemoryListener *kml, KVMSlot *slot, bool new)
{
    ...
    // printf("[set mem]: .slot: 0x%8x, .flags: 0x%8x, .gpa: 0x%16llx, .sz: 0x%16llx, .ua: 0x%16llx\n", mem.slot, mem.flags, mem.guest_phys_addr, mem.memory_size, mem.userspace_addr);
                             ...
                             MemoryRegionSection *section, bool add)
{
    ...
    if (mr->dual_mode || !memory_region_is_ram(mr)) {
    ...
    printf("%s: %s (%d)\n",__func__, memory_region_name(mr), mr->dual_mode);
...
int kvm_cpu_exec(CPUState *cpu)
{
    ...
    do {
        ...
        case KVM_EXIT_MMIO:
            DPRINTF("handle_mmio\n");
            // if (run->mmio.phys_addr > 0x2290000000)
...
int kvm_vm_ioctl__(KVMState *s, int type, ...)
{
    int ret;
    void *arg;
    va_list ap;
